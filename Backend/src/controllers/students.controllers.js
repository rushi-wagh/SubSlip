...
export const postStudents = asyncHandler(async(req,res) => {
    ...
    const newStudent = await new Student.create({
        name, rollNo, className, division, subjects,batch
    })
    if(!newStudent){
    ...
    const verifiedDb= await Verification.create({
        studentId: newStudent._id, coordinatorId: null,
...
export const updateVerificationStatus = asyncHandler(async(req,res) => {
    ...
    if(userRole !== 'ClassCoordinator'){
        throw new ApiError(400,"Only Class Coordinator can update verification status"  )
    }
    ...
    if(coordinator.className !== student.className || coordinator.division !== student.division){
        throw new ApiError(403,"You are not authorized to update verification status for this student")
    }
...
export const getVerifiedStudents = asyncHandler(async(req,res) => {
    const verifiedRecords = await Verification.find({verificationStatus: "Verified"}).populate('studentId');

    if(!verifiedRecords || verifiedRecords.length === 0){
        throw new ApiError(404,"No verified students found")
    }
    
    return res.status(200).json(new ApiResponse(200,verifiedRecords,"Verified students fetched successfully"))
})
